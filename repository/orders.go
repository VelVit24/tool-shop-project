...
func InsertOrder(db *sql.DB, id_user int, order models.Order) error {
	id := 0
	err := db.QueryRow("insert into Orders(id_user, description) values ($1, $2) returning id", id_user, order.Desc).Scan(&id)
	if err != nil {
		return err
	}
	carts, err := SelectCart(db, id_user)
	for _, cart := range carts {
		_, err := db.Exec("insert into OrderItems(id_order, id_instr, amount) values ($1, $2, $3)", id, cart.Id_instr, cart.Amount)
		if err != nil {
			log.Println(err)
		}
	}
